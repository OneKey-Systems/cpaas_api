...
import mx.onekey.dev.cpaas.api.domain.enums.IfMachine;
...
public class DialBuilder {
    ...
    /**
     * URL where some parameters specific to {@code <Dial>} will be sent for further processing. The calling party can
     * be redirected here upon the hangup of the B leg caller.
     * @param action
     * @return
     */
    ...
    /**
     * The duration in seconds a call made through {@code <Dial>} should occur for before ending. Default Value: 14400.
    ...
    /**
     * URL to an InboundXML document to be executed in place of the call ringtone
     * (a {@code <Say>} or {@code <Play>} would be appropriate in this document).
     * @param dialMusic
     * @return
     */
    ...
    /**
     * The URL that Zang should reach out to when the called party answers. The URL should return and InboundXML document
     * that contains {@code <Play>}, {@code <Pause>}, and/or {@code <Say>} elements only. Any other elements will be ignored.
     * @param confirmSound
     * @return
     */
    ...
    /**
     * Specifies digits that Zang should listen for and send to the callbackUrl if a caller inputs them. Separate
     * additional digits or digit patterns with a comma. Allowed Value: Pattern made up of the digits 0-9, #, or *.
     * @param digitsMatch
     * @return
     */
    ...
    /**
     * Boolean value specifying if call should be redirected to voicemail immediately. Note: only works if dialing TO a
     * mobile number. Default Value: false.
     * @param straightToVm
     * @return
     */
    ...
    /**
     * Specifies which stream of call audio to record. “in” to record the incoming caller audio, “out” to record the
     * outgoing caller audio, or “both” to record all audio on the call. “out” audio can only be captured if an outbound
     * {@code <Dial>} is performed during the call. “in” blocks any subsequent InboundXML elements until the inbound
     * audio recording is finished (via finishOnKey or timeout). Default Value: both.
